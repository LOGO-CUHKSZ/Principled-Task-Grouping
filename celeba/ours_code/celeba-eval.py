...
def train(params):
    ...
    optimizer = tf.keras.optimizers.legacy.SGD(params.lr, momentum=0.9)
    ...
    def cosine_sim_train_step(input, labels, first_step=False):
        with tf.GradientTape(persistent=True) as tape:
            ...
            task_gradients = {}
            for task in FLAGS.tasks:
                task_grads = tape.gradient(
                    task_permutations[task], ResBase.trainable_weights
                )
        ...
        for combined_task, task_gradient in combined_task_gradients:
            ...
            for task in FLAGS.tasks:
                ...
                stacked_filtered = tf.concat(
                    [tf.reshape(grad, shape=[-1]) for grad in filtered_grads], axis=0
                )
                stacked_task = tf.concat(
                    [tf.reshape(grad, shape=[-1]) for grad in task_gradients[task]],
                    axis=0,
                )
                cosine_sim = tf.reduce_sum(
                    tf.multiply(stacked_filtered, stacked_task)
    ...
    def train_uncertainty_step(input, labels, first_step=False):
        with tf.GradientTape(persistent=True) as tape:
            ...
            for task in FLAGS.tasks:
                clip_uncertainty = tf.clip_by_value(
                    uncertainty_weights[task], 0.01, 10.0
                )
                losses[task] = (
                    losses[task] / tf.exp(2 * clip_uncertainty) + clip_uncertainty
                )
            loss = tf.add_n(list(losses.values()))
        ...
        uncertainty_gradients = [
            val for val in tape.gradient(loss, list(uncertainty_weights.values()))
        ]
        optimizer.apply_gradients(
            zip(uncertainty_gradients, list(uncertainty_weights.values()))
        )
    ...
    def train_gradnorm_step(input, labels, first_step=False):
        with tf.GradientTape(persistent=True) as tape:
            ...
            post_weight_losses = {
                task: gradnorm_weights[task] * losses[task] for task in losses
            }
            gradnorms = {}
            for task in FLAGS.tasks:
                gradnorms[task] = tf.norm(
                    tape.gradient(
                        post_weight_losses[task], ResBase.trainable_weights[-3]
                    ),
                    ord=2,
                )
            ...
            gradnorm_losses = compute_gradnorm_losses(
                losses, gradnorm_l0, gradnorms, expected_gradnorm
            )

        # Update gradnorm weights.
        gradnorm_weight_grads = tape.gradient(
            gradnorm_losses, gradnorm_weights.values()
        )
        optimizer.apply_gradients(
            zip(gradnorm_weight_grads, list(gradnorm_weights.values()))
        )
        ...
        for task in FLAGS.tasks:
            gradnorm_weights[task].assign(
                tf.clip_by_value(gradnorm_weights[task], 0.1, 10.0)
            )
    ...
    def train_pcgrad_step(input, labels, first_step=False):
        ...
        base_gradvars = optimizer.compute_gradients(
            list(losses.values()), ResBase.trainable_weights
        )
        task_gradvars = [
            optimizer.compute_gradients([losses[task]], model.trainable_weights)
            for (task, model) in ResTowers.items()
        ]
        ...
        # Update the uncertainty weight variables.
        uw_gv = old_optimizer.compute_gradients(
            loss, list(uncertainty_weights.values())
        )
    ...
    for step in range(FLAGS.steps):
        ...
        batch_grad_metrics = {
            combined_task: {task: 0.0 for task in FLAGS.tasks}
        ...
        if len(model_params) == EARLY_STOP:
            model_params.pop(0)
...
for r in range(1, len(numbers) + 1):
    all_sub_combinations.extend(itertools.combinations(numbers, r))

result_folder = 'results/'

for group in all_sub_combinations:
    group_name = f'eval-{"-".join(str(i+1) for i in group)}'
    FLAGS.tasks = [TASKS[i] for i in group]
    FLAGS([""])
    if os.path.exists(os.path.join(result_folder, f"{group_name}.pkl")):
        continue
    # %%
    ...
    with open(os.path.join(result_folder, f"{group_name}.pkl"), "wb") as f:
        pickle.dump(eval_metrics_list, f)
