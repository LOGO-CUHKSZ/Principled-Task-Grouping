...
def main(args, group):
    default_tasks = "sdnkt"
    group_tasks = "".join([default_tasks[i] for i in group])
    args.tasks = group_tasks
    print(args)
...
if __name__ == '__main__':
    ...
    with open('taskonomy_groups.json') as f:
        taskonomy_groups = json.load(f)
    training_time = {}
    for group in taskonomy_groups:
        print(group)
        duration = main(args, group)
        training_time[tuple(group)] = duration
        print(group)
        print(training_time)
