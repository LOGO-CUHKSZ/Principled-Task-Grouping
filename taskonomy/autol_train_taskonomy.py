...
def main():
    ...
    model_ = models.__dict__[args.arch](tasks=losses.keys(),half_sized_output=args.half_sized_output)

    autol = AutoLambda(model, model_, criteria, len(taskonomy_tasks), args.autol_init)
    meta_weight_ls = np.zeros([args.epochs, len(taskonomy_tasks)], dtype=np.float32)
    ...
    model, optimizer, train_loader, val_loader, autol, meta_optimizer, meta_val_loader = accelerator.prepare(
        model, optimizer, train_loader, val_loader, autol, meta_optimizer, meta_val_loader
    )
    
    trainer=Trainer(accelerator,train_loader,val_loader,model,optimizer,criteria,args,losses.keys(),
        autol, meta_weight_ls, meta_optimizer, meta_val_loader,checkpoint)
    if args.validate:
...
class Trainer:
    def __init__(self,accelerator:Accelerator,train_loader,val_loader,model,optimizer,criteria,args,task_keys,
        autol, meta_weight_ls, meta_optimizer, meta_val_loader,checkpoint=None):
        ...
        self.autol = autol
        self.meta_weight_ls = meta_weight_ls
        self.meta_optimizer = meta_optimizer
        self.meta_val_loader = meta_val_loader
        if checkpoint:
    ...
    def train(self):
        for self.epoch in range(self.start_epoch,self.args.epochs):
            ...
            self.meta_weight_ls[self.epoch] = self.autol.meta_weights.detach().cpu()
    ...
    def train_epoch(self):
        ...
        for (train_input, train_target), (val_input, val_target) in zip(self.train_loader, self.meta_val_loader):
            if batch_num ==0:
                epoch_start_time2=time.time()
            ...
            loss_dict, loss = self.train_batch(train_input, train_target, val_input, val_target)
    ...
    def train_batch(self, train_input, train_target, val_input, val_target):
        self.meta_optimizer.zero_grad()
        self.autol.unrolled_backward(train_input, train_target, val_input, val_target,
                                get_average_learning_rate(self.optimizer), self.optimizer)
        ...
        train_loss_tmp = [w * loss_list[i] for i, w in enumerate(self.autol.meta_weights)]
