...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        grads = torch.Tensor(sum(grad_dims), len(self.selected_tasks)).to(self.device)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                loss = sum(loss)
                loss.backward()
                # for j in range(len(self.selected_tasks)):
                #     loss_tmp[j].backward(retain_graph=True)
                #     grad2vec(self.model, grads, grad_dims, j)
                #     self.model.zero_grad_shared_modules()
                # g = cagrad(grads, len(self.selected_tasks), 0.4, rescale=1)
                # overwrite_grad(self.model, g, grad_dims, len(self.selected_tasks))
                model_optim.step()
